...
class GraphDefHelper:
    ...
    def visualize(self, graph_def_filename, target_node_name, feed_dict=None):
        with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
            graph_def = load_graph_from_pbtxtfile('./' + graph_def_filename + '.pbtxt')
            g_in = tf.import_graph_def(graph_def, name='')

        train_writer = tf.summary.FileWriter('./log_for_tb')
        ...
        for feed_tensor in feed_dict:
            ...
            print(tensor)
            print(feed_tensor)
        sess.run(init_op, feed_dict=feeds)

        runs = 1
        for i in range(runs):
            sess.run(target, feed_dict=feeds)
        '''
